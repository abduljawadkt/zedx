      ...
      >
        ...
        <div>
          ...
          <div className="relative mt-8 flex flex-col gap-3 sm:flex-row">
            <Link
              href="/checkout"
              className="inline-flex h-13 items-center justify-center gap-2 rounded-full bg-[#00a0e3] px-5 type-control text-white shadow-lg shadow-[#00a0e3]/20 transition hover:scale-[1.02] hover:bg-[#008fcb] sm:h-14 sm:px-7"
            >
              View checkout
              <ArrowRight size={16} />
            </Link>
            <Link
              ...
              className="inline-flex h-13 items-center justify-center rounded-full border border-[#00a0e3]/30 bg-white px-5 type-control text-[#0078ad] transition hover:border-[#00a0e3] hover:bg-[#eaf8ff] sm:h-14 sm:px-7"
            >
              Shop chargers
            </Link>
          </div>
        ...
        <div className="relative mt-8 grid gap-4 sm:grid-cols-2 lg:mt-0 lg:grid-cols-1">
          <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.07] p-4 shadow-sm shadow-black/20 sm:rounded-[1.5rem]">
            ...
            <p className="mt-3 type-card-title">
              {activeCategory === "all"
                ? "All categories"
          ...
          <label className="relative block">
            ...
            <select
              ...
              className="mt-3 h-14 w-full appearance-none rounded-full border border-white/10 bg-black/25 px-5 pr-12 type-control text-white outline-none transition focus:border-[#00a0e3]/70"
            >
      ...
      <div className="mb-5 grid gap-3 rounded-[1.2rem] border border-white/10 bg-white/[0.03] p-3 lg:grid-cols-[1fr_auto_auto]">
        <label className="flex h-14 min-w-0 items-center gap-3 rounded-full border border-white/10 bg-black/25 px-5">
          ...
          <input
            ...
            className="min-w-0 flex-1 bg-transparent type-control text-white outline-none placeholder:text-white/38"
          />
        </label>
        <label className="relative block min-w-0 lg:min-w-56">
          ...
          <select
            ...
            className="h-14 w-full appearance-none rounded-full border border-white/10 bg-black/25 px-5 pr-12 type-control text-white outline-none transition focus:border-[#00a0e3]/70"
          >
      ...
      {visibleProducts.length > 0 ? (
        ...
        <div className="grid min-h-80 place-items-center rounded-[2rem] border border-white/10 bg-white/[0.03] p-8 text-center">
          <div>
            ...
            <h2 className="mt-4 type-section-title sm:text-4xl">
              No products match those filters.
            </h2>
            <button
              type="button"
              className="mt-6 rounded-full bg-white px-6 py-3 type-control text-[#050505]"
              onClick={() => {
...
function FilterPill({
  ...
  return (
    <button
      type="button"
      aria-pressed={active}
      className={`shrink-0 rounded-full border px-5 py-3 type-control transition ${
        active
          ? "border-[#00a0e3] bg-[#00a0e3] text-white shadow-lg shadow-[#00a0e3]/20"
          : "border-white/10 text-white/64 hover:border-white/30 hover:text-white"
      }`}
      onClick={onClick}
    >
      {label}
